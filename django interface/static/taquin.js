    ...
    $.fn.gameTaquin = function (image, params, success) {
        ...
        function taquin(generate, imageOriginalSize) {
            ...
            // G̩n̩rer les partis du taquin.
                        var unite = Math.ceil( params.width / gameStart.length );
            for (xi = 0; xi < gameStart.length; xi += 1) {
                for (yi = 0; yi < gameStart[0].length; yi += 1) {
                    ...
                    }).appendTo(generate).addClass(gameStart[xi][yi][1]).addClass("taquin-part").append('<h1>' + '</h1>');
                }
            }

            //donner numero pour les cases
            var indexing = 0 ;
            var ii = 0;
            var jj =0;
            var tt = params.length;
            var unite = Math.ceil( params.width / gameStart.length );
            var taquins = $('.taquin-part');
            for (ii = 0; ii < gameStart.length; ii++) {
                for (jj = 0; jj < gameStart.length; jj++) {
                    taquins.each(function () {
                        var p = $(this).position();
                        if ((p.top === (ii * unite)) && (p.left === (jj * unite))) {
                             $(this).find('h1').text(indexing);
                            indexing++;
                        }
                    });
                }
            ...
            // masuer dernier element
            var n = gameStart.length-1;
            var t = $(".taquin-" + n + "-" +n);
            $(t).find('h1').text('X');
            $(t).css("display",'none');
